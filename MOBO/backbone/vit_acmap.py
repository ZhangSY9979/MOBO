...
class Adapter(nn.Module):
    ...
    def forward_layer_feature(self, x, add_residual=True, residual=None):
        residual = x if residual is None else residual

        if self.layernorm_option == 'in':
            x = self.layer_norm_before(x)

        down_feature = self.down_proj(x)
        down = self.non_linear_func(down_feature)
        down = nn.functional.dropout(down, p=self.dropout, training=self.training)
...
class VisionTransformer(nn.Module):
    def __init__(self, config: Config):
        ...
        # setup adapter
        self.init_adapter = self.construct_adapter().requires_grad_(False)   # 每个block定义一个adapter
        self.setup_adapter()
    ...
    def setup_adapter(self):
        if self.config.our.init_random:  # 是否随机初始化
    ...
    def setup_adapter_fixed(self):
        self.cur_adapter = copy.deepcopy(self.init_adapter).requires_grad_(False)
    ...
    def forward_train(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            if self.config.transformer.vpt_on:  # NO
        ...
        if self.global_pool: # NO
            x = x[:, 1:, :].mean(dim=1)
            outcome = self.fc_norm(x)
        else:
            x = self.norm(x)
            outcome = x[:, 0]  # class token

        return outcome


    def forward_feature(self, x, adapter_list: List[Optional[nn.ModuleList]]):
        B = x.shape[0]
        x = self.patch_embed(x)
...
def vit_base_patch16_224_in21k(config: Config):
    ...
    checkpoint_model = timm.create_model('vit_base_patch16_224_in21k', pretrained=False, num_classes=0)

    checkpoint_model_temp = torch.load("../acmap/vit_model/vit_base_patch16_224_in21k.pth")

    del checkpoint_model_temp['head.weight']
    del checkpoint_model_temp['head.bias']


    checkpoint_model.load_state_dict(checkpoint_model_temp, strict=False)
