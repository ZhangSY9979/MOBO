...
class DataManager:
    ...
    def get_dataset(self, indices, source, mode, appendent=None, ret_data=False, m_rate=None, augmix=False):
        if source == 'train':
        ...
        if ret_data:
            ...
            return DummyDataset(data, targets, trsf, self.use_path, augmix)
...
augment_chain = transforms.Compose([
    transforms.Resize((224, 224)),  # 固定尺寸，保证 batch 可堆叠
    transforms.RandomRotation(degrees=15),
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3)
])

def AugMix(img, width=2, alpha=1.):
    ws = np.random.dirichlet([alpha] * width).astype(np.float32)
    img_aug = augment_chain(img)
    mix = torch.zeros_like(transforms.ToTensor()(img_aug))
    for i in range(width):
        img_aug = augment_chain(img)   # 每条链都执行所有操作
        mix += ws[i] * transforms.ToTensor()(img_aug)
    return mix


class DummyDataset(Dataset):
    def __init__(self, images, labels, trsf, use_path=False, augmix=False):
    ...
    def __getitem__(self, idx):
        ...
        # label = torch.tensor(self.labels[idx], dtype=torch.long)  # 转为 tensor
        ...
        if self.augmix:
            ...
            aug1 = AugMix(img)
            aug2 = AugMix(img)
            return idx, (clean, aug1, aug2), label
        else:
            # 普通 transform
            image = self.trsf(img)
